...
const startIndex = seedContent.indexOf('const cars = [');
const endIndex = seedContent.indexOf('for (const carData of cars)');

if (startIndex !== -1 && endIndex !== -1) {
  const newContent = seedContent.substring(0, startIndex) + newCars + '\n  ' + seedContent.substring(endIndex);
  fs.writeFileSync(seedPath, newContent, 'utf-8');
  console.log('Successfully updated seed.ts');
} else {
  console.log('Could not find injection points in seed.ts');
}
